...
def send_kobold_a_message(discord_message):
    ...
    # workaround for now to allow users to specify generating an image, atm does need the image keyword
    # can be improved for the ai to understand it needs to generate a message given the prompt/text input
    if "image" in user_message:
        # get a clean version for the image user wants then send it to comfyui
...
# allow full user input
while True: # Start the conversation
    user_message = input(f"{username}: ") # Get user input from the console
    send_kobold_a_message(user_message)
